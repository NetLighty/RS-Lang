  ...
  if (action.type === AddUserWordsActionsTypes.ADD_USER_WORDS_TO_STORE) {
    ...
    action.payload.forEach((item: IUserWord) => {
      if (item.optional) {
        ...
        if (!Array.isArray(userWords[item.optional.group][item.optional.page]))
          userWords[item.optional.group][item.optional.page] = [];
    ...
    if (action.payload.optional) {
      ...
      if (!Array.isArray(userWords[group][page])) {
        ...
        const index = userWords[group][page].findIndex(
          (item) => item.optional?.id === action.payload.optional?.id,
        );
        if (index >= 0) {
